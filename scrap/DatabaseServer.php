...
/**
 * <h4>Base database server abstract object.</h4>
 *
 ...
 * {@link DataSource::Path()} part of the connection string, these will be set as the
 * default database and collection, all operations will use these unless explicitly
 * indicated.
 *
 * This class features a generalised public interface form communicating with the server,
 * this is minimal and could be expanded by classes derived from concrete instances:
 *
 * <ul>
 * 	<li><b>{@link ListDatabases()}</b>: Return a list of databases on server.
 * 	<li><b>{@link ListCollections()}</b>: Return a list of databases on server.
 * 	<li><b>{@link DefaultDatabase()}</b>: Set, retrieve or reset default database.
 * 	<li><b>{@link DefaultCollection()}</b>: Set, retrieve or reset default collection.
 * 	<li><b>{@link GetDatabase()}</b>: Return native database object.
 * 	<li><b>{@link DropDatabase()}</b>: Drop a database.
 * 	<li><b>{@link GetCollection()}</b>: Return a native collection object.
 * 	<li><b>{@link DropCollection()}</b>: Drop a collecton.
 * 	<li><b>{@link FindDocument()}</b>: Retrieve first document of a selection.
 * 	<li><b>{@link FindDocuments()}</b>: Retrieve a selection of documents.
 * 	<li><b>{@link InsertDocument()}</b>: Insert a document.
 * 	<li><b>{@link InsertDocuments()}</b>: Insert a set of documents.
 * 	<li><b>{@link ReplaceDocument()}</b>: Replace a document.
 * 	<li><b>{@link UpdateDocument()}</b>: Update a document.
 * 	<li><b>{@link UpdateDocuments()}</b>: Update a set of documents.
 * 	<li><b>{@link DropDocument()}</b>: Drop a single document.
 * 	<li><b>{@link DropDocuments()}</b>: Drop a selection of documents.
 * </ul>
 *
 ...
 * // Use the native connection object...
 *	@example
 * $server = new Milko\PHPLib\Server( 'protocol://user:pass@host:9090/database/collection' );<br/>
 * $list = $server->findDocuments( $query );
 */
abstract class DatabaseServer extends Server
{
	/**
	 * <h4>Default database native object.</h4>
	 *
	 * This data member holds the <i>default database object</i>, it is the native object
	 * representing the default database for the current server.
	 *
	 * @var mixed
	 */
	protected $mDefaultDatabase = NULL;
	/**
	 * <h4>Default collection native object.</h4>
	 *
	 * This data member holds the <i>default collection object</i>, it is the native object
	 * representing the default collection for the current server.
	 *
	 * @var mixed
	 */
	protected $mDefaultCollection = NULL;
 ...
 *======================================================================================*/
	 ...
	 * @uses databasesList()
	 */
	public function ListDatabases( $doConnect = FALSE, $doNative = FALSE )
	{
		//
		// Assert connection.
		//
